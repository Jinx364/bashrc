...
GIT_PROMPT_STAGED="${BBlue}*"
GIT_PROMPT_CONFLICTS="${On_Red}x"
...
function set_prompt_beginend () {
	PROMPT_START="\n"
	PROMPT_END="$PC($White\w$PC)\n$PC($White\041\!$PC)\$ $ResetColor"
}
...
function setGitPrompt() {
	...
	if [ -n "$__CURRENT_GIT_STATUS" ]; then
	  ...
	  STATUS="$PC($ResetColor$STATUS$ResetColor$GIT_PROMPT_SUFFIX$PC)-"
